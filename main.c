...
void vfield(double t, double *x, int ndim, double *dx){
  ...
  dx[5]=J3*x[3]+J4*x[5];
  /*
  dx[2]=x[2]*(1.-3.*pow(x[0],2.)-pow(x[1],2.)) -x[4]*(1.+2.*x[0]*x[1]);
  dx[3]=x[3]*(1.-3.*pow(x[0],2.)-pow(x[1],2.)) - x[5]*(1.+2.*x[0]*x[1]);

  dx[4]=x[2]*(1.-2.*x[0]*x[1])               - x[4]*(1-3.*pow(x[1],2.)-pow(x[0],2.));
  dx[5]=x[3]*(1.-2.*x[0]*x[1])               - x[5]*(1-3.*pow(x[1],2.)-pow(x[0],2.));
  */
}
